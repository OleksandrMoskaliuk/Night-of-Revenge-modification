...
namespace NoRImmersiveEroMod
{
    ...
    [BepInDependency("org.bonedhg.plugins.nor_bonedmod", BepInDependency.DependencyFlags.HardDependency)]
    ...
    public class Plugin : global::BepInEx.BaseUnityPlugin
    {
        private void Awake()
        {
            ...
            LoggerMessage01 = "[Twitter @Dru9Dealer] Immersive mod enable";
        }

        private void Update()
        {
            if (GameplayInfo.mPlayerStatus != null && GameplayInfo.mPlayerStatus._BadstatusVal[0] > 0)
            {
                GameplayInfo.mPlayerStatus._BadstatusVal[0] -= 15 * Time.deltaTime;
                if (GameplayInfo.mPlayerStatus._BadstatusVal[0] < 0)
                {
                    GameplayInfo.mPlayerStatus._BadstatusVal[0] = 0;
                }
